...
def get_metric(dataset, t_freq, lon_area, lat_area, resolution, time_period, years, months, r_folder, r_filename, section_range, test):    
    ...
    if test:                                                                                                                            # for quickly testing some timesteps
        ...
        time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                                #
        da = da.sel(time=slice(f"{time_str1}", f"{time_str2}")).load()                                                                  #
    time_axis = da.time.data                                                                                                            # this time axis is given to section result 
    # -- get metric for given (years, months) --
    print('getting metric ..')
    metric = []                                                                                                                         # collect all results
    for i, (year, month) in enumerate(zip(years, months)):                                                                              # loop through months
        time_str = f"{year}-{int(month):02d}"
        da_month = da.sel(time=slice(f"{time_str}", f"{time_str}")).load() 
        for day in np.arange(0, len(da_month['time'])):                                                                                             # loop through days
            da_day = da_month.isel(time = day)
            data_objects = [da_day, i, lon_area, lat_area]                                                                              
            metric.append(mC.calculate_metric(data_objects))
        print(f'finished year: {year} month: {month}')
    ds = xr.concat(metric, dim='time') 
    ds = ds.drop_vars('time')                                                                                                           # When concatenating the time coordiante becomes a numpy array
    ds = ds.assign_coords(time = time_axis)                                                                                             # assign the original time coordinates for section
    # -- save result from section --
    print(f'saving results from model: {dataset}')
