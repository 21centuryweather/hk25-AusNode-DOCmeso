...
def get_weights(da_in, simulation_id, x_res, y_res, folder_scratch, switch_area, full_domain):
...
def convert_grid(ds_in, folder_scratch, path_gridDes, path_weights):
...
def conservatively_interpolate(da_in, res, switch_area, simulation_id, full_domain = True):
    ...
    x_res = res
    y_res = res
    path_gridDes, path_weights = get_weights(da_in, simulation_id, x_res, y_res, folder_scratch, switch_area, full_domain)

    # -- grid conversion --
    ds_in = xr.Dataset(data_vars = {da_in.name: da_in}) 
    ds_out = convert_grid(ds_in, folder_scratch, path_gridDes, path_weights)
